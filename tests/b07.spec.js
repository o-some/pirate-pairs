...
async function waitReady(page,timeout=25000){
  await expect.poll(async()=>{
    ...
    return turn==='DU BIST DRAN'&&!bannerClass.includes('show')&&!splashClass.includes('show')&&transient===0&&open===0&&resultClass.includes('hidden');
  },{timeout}).toBe(true);
}
...
async function playMismatch(page,{waitForReady=true}={}){
  ...
  const second=page.locator(`#grid .card[data-index="${pair[1]}"]`);
  await first.click();
  await expect(first).toHaveClass(/flipped/,{timeout:4000});
  await second.click();
  await expect(second).toHaveClass(/flipped/,{timeout:4000});
  if(waitForReady)await waitReady(page,30000);
  return true;
}
...
async function finishVarkos(page){
  for(let turn=0;turn<20;turn++){
    ...
    if(!(await playPair(page))){
      const advanced=await playMismatch(page);
      expect(advanced).toBe(true);
    }
  }
  await expect(page.locator('#result')).not.toHaveClass(/hidden/,{timeout:20000});
  await expect(page.locator('#result')).toHaveClass(/stage-complete/,{timeout:5000});
}
...
test.describe('B07 stage data and compact mobile layout',()=>{
  ...
  test('A1→C2 tiers use real different decks and C2 remains compact',async({page})=>{
    ...
    await expect(page.locator('#help p')).not.toContainText(['Muschelblick']);
    await playPair(page);
...
test.describe('B07 boss clarity',()=>{
  ...
  test('Brax uses an explosive TNT barrel and current boss is explicit below',async({page})=>{
    ...
    const bg=await page.locator('#bossRoadmap .boss-road-item[data-boss-id="2"] .boss-road-num').evaluate(el=>getComputedStyle(el).backgroundImage);
...
test.describe('B07 epic Varkos and progression',()=>{
  ...
  test('A1 Varkos fight explains phases, wins epically and restarts at A2 Boss 1',async({page})=>{
