(() => {
  ...
  let aiMemory=new Map(), playerSeen=new Set(), peekUsed=false, gameOver=false, toastTimer, playerAttempts=0, lastAbilityAttempt=-1, lastOutcome='loss';
  ...
  function applyBossUi(){
    ...
    if(levelTag)levelTag.innerHTML=`<span>☠</span> LEVEL ${b.bossId} · ${b.location||b.name.toUpperCase()}`;
  ...
  function markMatched(indices,owner){
    ...
    indices.forEach(i=>{fogged.delete(i);chained.delete(i);if(cards[i].id===cursedCardId)cursedCardId=null;cards[i].matched=true;cards[i].owner=owner;const el=cardEl(i);if(!el)return;el.classList.remove('flipped','bomb-armed','fogged','chained','cursed-memory');el.classList.add('matched',owner==='ai'?'kai':'tula');el.setAttribute('aria-disabled','true');const lab=el.querySelector('.matched-by');if(lab)lab.textContent=owner==='player'?'✓ TULA':`☠ ${bossLabel()}`;});
  }
  ...
  async function animateSwap(a,b,tone='swap'){
    ...
    await Promise.all([aa?.finished?.catch(()=>{}),ab?.finished?.catch(()=>{})]);[cards[a],cards[b]]=[cards[b],cards[a]];reindexMemory();render();return true;
  }
  async function swapHiddenCards(){const opts=available().filter(i=>!fogged.has(i)&&!chained.has(i));if(opts.length<2)return false;const[a,b]=shuffle(opts).slice(0,2);await banner('KAI MISCHT DIE KARTEN!','Beobachte genau, welche zwei verdeckten Karten ihre Plätze tauschen.','swap',650);await animateSwap(a,b);await banner('PLÄTZE GETAUSCHT','Die beiden Karten liegen jetzt an ihren neuen Positionen.','swap done',560);return true;}
  ...
  async function plantBomb(){const opts=available().filter(i=>!fogged.has(i)&&!chained.has(i));if(!opts.length)return false;clearBomb();bombIndex=shuffle(opts)[0];bombExpiresAt=playerAttempts+2;const el=cardEl(bombIndex);el?.classList.add('bomb-targeting');await banner('BRAX LEGT EINE BOMBE!','Merk dir die markierte Karte. Öffnest du sie: −1 für dich, +1 für Brax.','bomb',1000);el?.classList.remove('bomb-targeting');el?.classList.add('bomb-armed');return true;}
  async function triggerBomb(i){if(i!==bombIndex)return;const el=cardEl(i);el?.classList.remove('bomb-armed');el?.classList.add('bomb-explode');bombIndex=null;bombExpiresAt=null;const lost=scores.player>0?1:0;if(lost)scores.player--;scores.ai++;updateHud();await banner('BOMBE EXPLODIERT!',lost?'Du verlierst 1 Punkt. Brax bekommt 1 Punkt.':'Du hattest noch keinen Punkt – Brax bekommt trotzdem +1.','bomb explode',650);el?.classList.remove('bomb-explode');}

  function clearFog(announce=false){if(!fogged.size)return;fogged.forEach(i=>{const el=cardEl(i);el?.classList.remove('fogged','fog-denied');if(el&&!cards[i]?.matched&&!chained.has(i))el.removeAttribute('aria-disabled');});fogged=new Set();fogExpiresAt=null;if(announce)showToast('🌫 Der Nebel lichtet sich.','good');}
  ...
  async function castMemoryCurse(){const opts=available().filter(i=>playerSeen.has(cards[i].id)&&!fogged.has(i)&&!chained.has(i));if(!opts.length){await banner('RODERICK SUCHT DEINE ERINNERUNG','Noch kennt er keine passende Karte von dir.','curse',650);return false;}clearCurse();const i=shuffle(opts)[0];cursedCardId=cards[i].id;const el=cardEl(i);el?.classList.add('curse-forming');await banner('RODERICKS FLUCH!','Eine bereits bekannte Karte wurde verflucht. Öffnest du sie, verschiebt er eine andere Erinnerung.','curse',1000);el?.classList.remove('curse-forming');el?.classList.add('cursed-memory');return true;}
  async function triggerMemoryCurse(triggerIndex){
    if(cards[triggerIndex]?.id!==cursedCardId)return;clearCurse();
    const known=available().filter(i=>i!==triggerIndex&&playerSeen.has(cards[i].id)&&!fogged.has(i)&&!chained.has(i));
    if(!known.length){await banner('FLUCH GEBROCHEN','Roderick findet keine weitere bekannte Karte zum Verschieben.','curse',600);return;}
    const from=shuffle(known)[0];const targets=available().filter(i=>i!==triggerIndex&&i!==from&&!fogged.has(i)&&!chained.has(i));if(!targets.length)return;const to=shuffle(targets)[0];
    await banner('VERFLUCHTE ERINNERUNG!','Beobachte: Roderick verschiebt eine andere Karte, die du bereits kanntest.','curse',720);await animateSwap(from,to,'curse');await banner('ERINNERUNG VERSCHOBEN','Die verfluchte Bewegung ist abgeschlossen.','curse',520);
  }
  ...
  function clearChains(announce=false){if(!chained.size)return;chained.forEach(i=>{const el=cardEl(i);el?.classList.remove('chained','chain-denied');if(el&&!cards[i]?.matched&&!fogged.has(i))el.removeAttribute('aria-disabled');});chained=new Set();chainExpiresAt=null;if(announce)showToast('⛓ Ironhooks Ketten brechen.','good');}
  async function castChains(){const opts=available().filter(i=>!fogged.has(i));if(opts.length<3)return false;clearChains();const picks=shuffle(opts).slice(0,2);chained=new Set(picks);chainExpiresAt=playerAttempts+2;picks.forEach(i=>cardEl(i)?.classList.add('chain-forming'));await banner('IRONHOOK FESSELT DIE KARTEN!','Zwei Karten sind für deine nächsten 2 Versuche gesperrt. Ironhook selbst kann sie weiterhin nutzen.','chain',1050);picks.forEach(i=>{const el=cardEl(i);el?.classList.remove('chain-forming');el?.classList.add('chained');el?.setAttribute('aria-disabled','true');});return true;}
  ...
  async function maybeAbility(){if(gameOver||selected.length)return;await cleanExpired();const a=ability();if(!a?.type||a.type==='none')return;const every=Math.max(1,Number(a.everyPlayerAttempts||3));if(!playerAttempts||playerAttempts%every!==0||lastAbilityAttempt===playerAttempts)return;lastAbilityAttempt=playerAttempts;lock=true;setTurnUi();try{if(a.type==='swap')await swapHiddenCards();else if(a.type==='bomb')await plantBomb();else if(a.type==='fog')await castFog();else if(a.type==='memory-curse')await castMemoryCurse();else if(a.type==='tribute')await startTribute();else if(a.type==='chains')await castChains();}finally{lock=false;setTurnUi();}}
  async function enterPlayerTurn(){turn='player';lock=true;setTurnUi();await maybeAbility();if(!gameOver){lock=false;setTurnUi();}}
  ...
  async function resolveSelection(actor){
    ...
    if(match){markMatched([a,b],actor);matchedPairs[actor]++;scores[actor]++;if(actor==='player')satisfyTribute();updateHud();const learned=pairText([a,b]);showToast(actor==='player'?`✓ Stark! ${learned}`:`☠ ${boss().name} schnappt sich: ${learned}`,actor==='player'?'good':'bad');selected=[];if(actor==='player')playerAttempts++;await cleanExpired();if(checkGameEnd())return;if(actor==='ai'){lock=false;setTurnUi();await sleep(520);aiTurn();}else await enterPlayerTurn();}
    else{showToast(actor==='player'?`Kein Paar – ${boss().name} ist dran.`:`${boss().name} liegt daneben – dein Zug!`,actor==='player'?'bad':'good');await sleep(actor==='player'?550:430);hide(a);hide(b);selected=[];if(actor==='player')playerAttempts++;await cleanExpired();if(actor==='player'){turn='ai';lock=true;setTurnUi();await sleep(Number(boss().thinkingDelay??720));aiTurn();}else await enterPlayerTurn();}
  }
  ...
  async function aiTurn(){if(gameOver)return;turn='ai';lock=true;setTurnUi();cleanMemory();await sleep(360);const pair=knownPair();let first=pair?pair[0]:randomAvailable();if(first==null){await enterPlayerTurn();return;}reveal(first,'ai');selected=[first];await sleep(780);let second=pair?pair[1]:rememberedMate(cards[first].pairId,first);if(second==null||cards[second].matched||isOpen(second))second=randomAvailable([first]);if(second==null){hide(first);selected=[];await enterPlayerTurn();return;}reveal(second,'ai');selected.push(second);await resolveSelection('ai');}

  async function usePeek(){if(peekUsed||lock||turn!=='player'||selected.length||gameOver)return;const opts=available().filter(i=>!fogged.has(i)&&!chained.has(i));if(opts.length<2)return;peekUsed=true;peekBtn.disabled=true;peekBtn.textContent='VERBRAUCHT';lock=true;setTurnUi();const picks=shuffle(opts).slice(0,2);picks.forEach(i=>cardEl(i)?.classList.add('peek'));showToast('🐚 Muschelblick: Merk dir diese beiden Karten!','good');await sleep(1350);picks.forEach(i=>cardEl(i)?.classList.remove('peek'));lock=false;setTurnUi();}
  function checkGameEnd(){
    if(matchedPairs.player+matchedPairs.ai<PAIRS.length)return false;gameOver=true;lock=true;setTurnUi();const win=scores.player>scores.ai,tie=scores.player===scores.ai;lastOutcome=win?'win':tie?'tie':'loss';const shells=win?20:tie?10:5,xp=win?60:tie?40:25,b=boss(),next=BOSSES[bossIndex+1];
    resultTitle.textContent=win?`Du hast ${b.name} geschlagen!`:tie?'Unentschieden auf hoher See!':`${b.name} gewinnt diese Runde.`;
    resultCopy.textContent=win?`${b.name} ist besiegt. ${next?`Als Nächstes wartet ${next.name}.`:'Du hast alle Bosse dieses Entwicklungs-Batches geschlagen.'}`:tie?'Keiner gewinnt das Duell. Noch eine Runde entscheidet es.':`${b.name} hatte diesmal die besseren Tricks. Beim nächsten Duell kennst du seine Mechanik.`;
    resultPlayer.textContent=String(scores.player);resultShells.textContent=`+${shells}`;resultXp.textContent=`+${xp}`;resultTag.textContent=win?'✓ BOSS BESIEGT':tie?'⚔ GLEICHSTAND':'☠ BOSS GEWINNT';if(againBtn)againBtn.textContent=win&&next?`⚔ WEITER ZU ${next.name.toUpperCase()}`:'↻ NOCH EIN DUELL';if(resultNote)resultNote.textContent=win&&next?`NÄCHSTE ETAPPE: ${next.name.toUpperCase()}`:win?'ALLE BOSSE DIESES BATCHES BESIEGT':`NOCH EIN DUELL GEGEN ${b.name.toUpperCase()}`;setTimeout(()=>result.classList.remove('hidden'),900);return true;
  }
  ...
  startBtn?.addEventListener('click',()=>{intro.classList.add('hidden');lock=false;setTurnUi();showToast(`Besiege ${boss().name} und merk dir die Übersetzungen.`,'good');});
