(() => {
  ...
  async function castMemoryCurse(){const opts=available().filter(i=>playerSeen.has(cards[i].id)&&!fogged.has(i)&&!chained.has(i));if(opts.length<1){await banner('RODERICK SUCHT DEINE ERINNERUNG','Er braucht mindestens eine bekannte, verdeckte Karte für seinen Fluch.','curse',650);return false;}clearCurse();const i=shuffle(opts)[0];cursedCardId=cards[i].id;const el=cardEl(i);el?.classList.add('curse-forming');await banner('RODERICKS FLUCH!','Eine bereits bekannte Karte wurde verflucht. Öffnest du sie, verschiebt er eine andere verdeckte Karte.','curse',1000);await waitForHelpClosed();el?.classList.remove('curse-forming');el?.classList.add('cursed-memory');return true;}
  ...
  async function castChains(){const royal=isRoyalChaos(),opts=available().filter(i=>!fogged.has(i));if(opts.length<3)return false;clearChains();const picks=shuffle(opts).slice(0,2);chained=new Set(picks);chainExpiresAt=playerAttempts+(royal?1:2);picks.forEach(i=>cardEl(i)?.classList.add('chain-forming'));await banner(royal?'VARKOS SCHLIESST DIE KRONENKETTEN!':'IRONHOOK FESSELT DIE KARTEN!',royal?'Zwei Karten sind für diesen Versuch gesperrt. Varkos selbst kann sie weiterhin erreichen.':'Zwei Karten sind für deine nächsten 2 Versuche gesperrt. Ironhook selbst kann sie weiterhin nutzen.',royal?'varkos phase-2 chain':'chain',1050);await waitForHelpClosed();picks.forEach(i=>{const el=cardEl(i);el?.classList.remove('chain-forming');el?.classList.add('chained');el?.setAttribute('aria-disabled','true');});return true;}
