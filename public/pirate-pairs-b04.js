(() => {
  ...
  async function castMemoryCurse(){const known=available().filter(i=>playerSeen.has(cards[i].id)&&!fogged.has(i)&&!chained.has(i));const fallback=available().filter(i=>!fogged.has(i)&&!chained.has(i));const opts=known.length?known:fallback;if(!opts.length)return false;clearCurse();const i=shuffle(opts)[0];cursedCardId=cards[i].id;const el=cardEl(i);el?.classList.add('curse-forming');await banner('RODERICKS FLUCH!',known.length?'Eine bereits bekannte Karte wurde verflucht. Öffnest du sie, verschiebt er eine andere verdeckte Karte.':'Keine bekannte Karte war verfügbar – Roderick verflucht stattdessen eine andere verdeckte Karte.','curse',1000);await waitForHelpClosed();el?.classList.remove('curse-forming');el?.classList.add('cursed-memory');return true;}
