(() => {
  ...
  function resetStates(){
    ...
    cannonTargets=new Set(); cannonTouched=false; cannonExpiresAt=null; shadowIndex=null; shadowExpiresAt=null;
    document.body.classList.remove('tribute-active'); hideBanner();
  }
  ...
  async function shiftLine(){const lines=lineCandidates();if(!lines.length){await banner('CORVIN PRÜFT DAS DECK','Keine vollständige verdeckte Reihe oder Spalte ist gerade frei.','shift',620);return false;}const line=shuffle(lines)[0];const els=line.map(cardEl);const rects=els.map(el=>el?.getBoundingClientRect());if(els.some(el=>!el)||rects.some(r=>!r))return false;await banner('CORVIN ORDNET DAS DECK NEU!','Beobachte die vier verdeckten Karten: Eine ganze Reihe oder Spalte rückt um eine Position.','shift',760);const animations=els.map((el,k)=>{el.classList.add('corvin-shifting');const dest=rects[(k+1)%4],src=rects[k],dx=dest.left-src.left,dy=dest.top-src.top;return el.querySelector('.back')?.animate([{transform:'translate(0,0)'},{transform:`translate(${dx}px,${dy}px)`}],{duration:760,easing:'cubic-bezier(.22,.75,.18,1)',fill:'forwards'});});await Promise.all(animations.map(a=>a?.finished?.catch(()=>{})));const before=line.map(i=>cards[i]);line.forEach((idx,k)=>{cards[line[(k+1)%4]]=before[k];});reindexMemory();render();await banner('REIHE VERSCHOBEN','Die vier Karten liegen jetzt an ihren neuen Positionen.','shift done',520);return true;}
  ...
  async function castShadow(){const opts=available().filter(i=>!fogged.has(i)&&!chained.has(i));if(!opts.length)return false;clearShadow();shadowIndex=shuffle(opts)[0];shadowExpiresAt=playerAttempts+1;const el=cardEl(shadowIndex);el?.classList.add('shadow-arriving');await banner('AZRAK RUFT DEN SCHATTEN!','Eine Karte verschwindet im Schatten und ist für dich blockiert. Nach deiner ersten Karte wandert der Schatten weiter.','shadow',980);el?.classList.remove('shadow-arriving');el?.classList.add('shadowed');el?.setAttribute('aria-disabled','true');return true;}
  async function moveShadow(){if(shadowIndex==null)return;const old=shadowIndex;const opts=available().filter(i=>i!==old&&!selected.includes(i)&&!fogged.has(i)&&!chained.has(i));if(!opts.length)return;const next=shuffle(opts)[0],oldEl=cardEl(old),nextEl=cardEl(next);oldEl?.classList.add('shadow-leaving');nextEl?.classList.add('shadow-arriving');await sleep(460);if(oldEl&&!cards[old]?.matched&&!fogged.has(old)&&!chained.has(old))oldEl.removeAttribute('aria-disabled');oldEl?.classList.remove('shadowed','shadow-leaving');shadowIndex=next;nextEl?.classList.remove('shadow-arriving');nextEl?.classList.add('shadowed');nextEl?.setAttribute('aria-disabled','true');showToast('Der Schatten ist auf eine andere Karte gewandert.','bad');}
  ...
  async function maybeAbility(){if(gameOver||selected.length)return;await cleanExpired();const a=ability();if(!a?.type||a.type==='none')return;const every=Math.max(1,Number(a.everyPlayerAttempts||3));if(!playerAttempts||playerAttempts%every!==0||lastAbilityAttempt===playerAttempts)return;lastAbilityAttempt=playerAttempts;lock=true;setTurnUi();try{if(a.type==='swap')await swapHiddenCards();else if(a.type==='bomb')await plantBomb();else if(a.type==='fog')await castFog();else if(a.type==='memory-curse')await castMemoryCurse();else if(a.type==='tribute')await startTribute();else if(a.type==='chains')await castChains();else if(a.type==='cannon')await castCannon();else if(a.type==='line-shift')await shiftLine();else if(a.type==='shadow')await castShadow();}finally{lock=false;setTurnUi();}}
  ...
  async function onPlayerCard(i){
    ...
    if(selected.length===1&&shadowIndex!=null&&ability()?.type==='shadow'){lock=true;setTurnUi();await moveShadow();}
  ...
  async function resolveSelection(actor){
    ...
    if(actor==='player'&&cannonTargets.size)await resolveCannon(match);
